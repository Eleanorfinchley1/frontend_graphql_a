defmodule Web.NotificationViewTest do
  ...
  describe "device view" do
    ...
    test "template \"index.json\" renders", %{conn: conn, tokens: tokens} do
      ...
      for _ <- 1..75, do: insert(:notification, recipient: owner)
      ...
      resp =
        conn
        |> authenticate(hd(tokens))
        |> get(Routes.notification_path(conn, :index, page: 2))
        |> response(200)
        |> Jason.decode!()
      ...
      resp =
        conn
        |> authenticate(hd(tokens))
        |> get(Routes.notification_path(conn, :index, page: 8))
        |> response(200)
        |> Jason.decode!()
