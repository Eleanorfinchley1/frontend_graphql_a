defmodule Web.InterestViewTest do
  use Web.ConnCase, async: true
  import BillBored.Factory

  describe "interest view" do
    setup [:create_users]
    ...
    test "template \"index.json\" renders", %{conn: conn, tokens: tokens} do
      ...
      for _ <- 1..75, do: insert(:user_interest, user: owner)
      ...
      resp =
        conn
        |> authenticate(hd(tokens))
        |> get(Routes.interest_path(conn, :index, page: 2))
        |> response(200)
        |> Jason.decode!()
      ...
      resp =
        conn
        |> authenticate(hd(tokens))
        |> get(Routes.interest_path(conn, :index, page: 8))
        |> response(200)
        |> Jason.decode!()
